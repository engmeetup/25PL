...
function initQuiz() {
    ...
    selectedQuestion.answers.forEach((answer) => {
        ...
        btn.className = 'option-btn w-full text-left bg-white/5 hover:bg-white/10 active:bg-white/20 active:scale-[0.98] border border-white/5 rounded-xl p-4 transition-all duration-200 text-slate-200 group flex items-center justify-between shadow-sm';
        
        btn.innerHTML = `
            <span class="text-base md:text-lg font-medium drop-shadow-sm">${answer.text}</span>
            <div class="indicator w-6 h-6 rounded-full border-2 border-slate-500/50 group-hover:border-cyan-400 bg-transparent transition-colors duration-300 flex items-center justify-center">
...
function checkAnswer(isCorrect, btnElement) {
    ...
    if (isCorrect) {
        ...
        errorMsg.innerText = "පිළිතුර වැරදියි. ඔබට වෙනත් ප්‍රශ්නයක් ලබාදේවි...";
        ...
        setTimeout(() => {
            errorMsg.classList.add('hidden');
            // reset error message text for any future UI needs if relevant, though it gets overwritten above
            errorMsg.innerText = "පිළිතුර වැරදියි. කරුණාකර නැවත උත්සාහ කරන්න!";
