...
const translations = {
    ...
    si: {
        ...
        locationTitle: "හමුව පැවැත්වෙන ස්ථානය",
        backBtn: "ආපසු",
    ...
    ta: {
        ...
        locationTitle: "கூட்டம் நடைபெறும் இடம்",
        backBtn: "திரும்பு",
        fontClass: "font-sans"
    }
};
...
function selectLanguage(lang) {
    ...
    document.getElementById('location-title').innerText = translations[lang].locationTitle;
    
    // Switch fonts based on language
    const elementsToUpdateFont = [
        'main-title', 'main-subtitle', 'question-text', 
        'error-msg', 'success-title', 'success-msg', 'btn-text', 'location-title'
    ];
...
function showLocationView() {
    // Inject map src if not already loaded (Lazy load)
    ...
    const venueImg = document.getElementById('venue-image');
    if (venueImg && !venueImg.panzoomInstance && typeof panzoom !== 'undefined') {
        venueImg.panzoomInstance = panzoom(venueImg, {
            maxZoom: 5,
            minZoom: 0.5,
            bounds: true,
            boundsPadding: 0.1
    ...
    setTimeout(() => {
        ...
        // Show location view
        const locContainer = document.getElementById('location-view-container');
        locContainer.classList.remove('hidden');
        locContainer.classList.add('flex', 'fade-in');
        
        // Wait for next frame before setting opacity to trigger transition
        requestAnimationFrame(() => {
            locContainer.style.opacity = '1';
        });
    }, 300);
}
